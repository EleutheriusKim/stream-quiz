...
public class Quiz4 {
    ...
    public Integer[] quiz7() {
        return new Integer[]{
					transactions.stream().map(Transaction::getValue).reduce(0, (c1, c2) -> c1 > c2 ? c1 : c2),
					transactions.stream().min(Comparator.comparingInt(Transaction::getValue)).map(Transaction::getValue).orElse(0)
				};
    }
